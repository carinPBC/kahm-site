...
export async function onRequest(context) {{
  ...
  context.waitUntil(
    fetch(`${https://pbc-cms-production.up.railway.app}/api/track/click?station=${kahm}&type=${{encodeURIComponent(type)}}&label=${{encodeURIComponent(label)}}&url=${{encodeURIComponent(dest)}}&page=${{encodeURIComponent(page)}}`)
      .catch(() => {{}})
  );
